...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["foto"]["name"])) {
        $tipos_permitidos = array('image/jpeg', 'image/png', 'image/jpg', 'image/HEIC');
        ...
        if (in_array($_FILES["foto"]["type"], $tipos_permitidos) && $_FILES["foto"]["size"] <= $tamanho_maximo) {
            ...
            if (!move_uploaded_file($_FILES["foto"]["tmp_name"], $foto_caminho)) {
                echo json_encode(array('status' => 'error', 'message' => 'Erro ao fazer upload da foto.'));
                exit; // Encerra a execução
            }

            $foto = $foto_caminho;
        } else {
            echo json_encode(array('status' => 'error', 'message' => 'Arquivo de foto inválido.'));
            exit;
        }
    }
